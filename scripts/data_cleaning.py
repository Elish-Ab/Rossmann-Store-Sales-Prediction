...
try:
    ...
    test['CompetitionDistance'] = test['CompetitionDistance'].fillna(train['CompetitionDistance'].max())
...
try:
    ...
    train['CompetitionActive'] = (
        ...
        ((train['CompetitionOpenSinceYear'] == train['Year']) & (train['CompetitionOpenSinceMonth'] <= train['Month']))
    ).astype(int)

    test['CompetitionActive'] = (
        (test['CompetitionOpenSinceYear'] < test['Year']) |
        ((test['CompetitionOpenSinceYear'] == test['Year']) & (test['CompetitionOpenSinceMonth'] <= test['Month']))
    ).astype(int)

    # Promo2 active flag
    train['Promo2Active'] = (
        (train['Promo2SinceYear'] < train['Year']) |
        ((train['Promo2SinceYear'] == train['Year']) & (train['Promo2SinceWeek'] <= train['WeekOfYear']))
    ).astype(int)

    test['Promo2Active'] = (
        (test['Promo2SinceYear'] < test['Year']) |
        ((test['Promo2SinceYear'] == test['Year']) & (test['Promo2SinceWeek'] <= test['WeekOfYear']))
    ).astype(int)
